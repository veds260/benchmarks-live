...
export async function POST(req: NextRequest) {
  ...
  const existing = db.prepare("SELECT COUNT(*) as c FROM entries").get() as { c: number };
  if (existing.c > 0) {
    return NextResponse.json({ message: `Already seeded with ${existing.c} entries`, seeded: false });
  }
  ...
  return NextResponse.json({ message: `Seeded ${count.c} entries with benchmarks and history`, seeded: true });
}
